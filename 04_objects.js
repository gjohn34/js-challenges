...
const productById = (productId) => {
  ...
  return shop.products.find(function(object) {return object.id == productId})
}
...
const formatAddress = (email) => {
  ...
  address = shop.customers[email].address// Given a user's email, return their address in the format:
...
const totalCost = (email, orderId) => {
  let allCost = 0
  items = shop.customers[email].orders.find(function(order) { return orderId == order.id }).items

  items.forEach(function(item) {
    allCost += productById(item.productId).price * item.qty
  })
  return allCost
